...
export async function downloadResultImpl(result: Status): Promise<void> {
  ...
  for (let i = 0; i < result.candidates.candidates2.length; i++) {
    sheet2.row(i + 2).cell(1).value(result.candidates.candidates2[i].name);
    sheet2.row(i + 2).cell(2).value(result.candidates.candidates2[i].votes);
    sheet2.row(i + 2).cell(3).value(result.candidates.candidates2[i].firstGradeVotes);
    sheet2.row(i + 2).cell(4).value(result.candidates.candidates2[i].secondGradeVotes);
    sheet2.row(i + 2).cell(5).value(result.candidates.candidates2[i].thirdGradeVotes);
  }
  ...
  for (let i = 0; i < result.candidates.candidates1M.length; i++) {
    sheet1M.row(i + 2).cell(1).value(result.candidates.candidates1M[i].name);
    sheet1M.row(i + 2).cell(2).value(result.candidates.candidates1M[i].votes);
    sheet1M.row(i + 2).cell(3).value(result.candidates.candidates1M[i].firstGradeVotes);
    sheet1M.row(i + 2).cell(4).value(result.candidates.candidates1M[i].secondGradeVotes);
    sheet1M.row(i + 2).cell(5).value(result.candidates.candidates1M[i].thirdGradeVotes);
  }
  ...
  for (let i = 0; i < result.candidates.candidates1F.length; i++) {
    sheet1F.row(i + 2).cell(1).value(result.candidates.candidates1F[i].name);
    sheet1F.row(i + 2).cell(2).value(result.candidates.candidates1F[i].votes);
    sheet1F.row(i + 2).cell(3).value(result.candidates.candidates1F[i].firstGradeVotes);
    sheet1F.row(i + 2).cell(4).value(result.candidates.candidates1F[i].secondGradeVotes);
    sheet1F.row(i + 2).cell(5).value(result.candidates.candidates1F[i].thirdGradeVotes);
  }
